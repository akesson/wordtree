...
impl NodeId {
    ...
    unsafe fn from_index(n: usize) -> Self {
        NodeId(unsafe { NonZeroU32::new_unchecked(n as u32 + 1) })
    }
...
fn _static_assert_size_of_node() {
    ...
    let _ = std::mem::transmute::<Node<()>, [u32; 5]>;
}
...
impl<'a, T: 'a> PartialEq for NodeRef<'a, T> {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id && ptr::eq(self.tree, other.tree) && ptr::eq(self.node, other.node)
    }
}
...
impl<T> Tree<T> {
    ...
    /// Returns a reference to the specified node.
    #[inline]
    pub fn get_unchecked(&self, id: NodeId) -> NodeRef<'_, T> {
        NodeRef {
            id,
            node: self.node(id),
            tree: self,
    ...
    /// Returns a mutator of the specified node.
    #[inline]
    pub fn get_unchecked_mut(&mut self, id: NodeId) -> NodeMut<'_, T> {
    ...
    #[inline]
    pub fn val_ref(&self, id: NodeId) -> &'_ T {
        unsafe { &self.vec.get_unchecked(id.to_index()).value }
    }

    /// get a reference to the underlying value
    #[inline]
    pub fn val_mut(&mut self, id: NodeId) -> &'_ mut T {
        unsafe { &mut self.vec.get_unchecked_mut(id.to_index()).value }
    }

    /// get the first and last child
    #[inline]
    pub fn first_and_last_child(&self, id: NodeId) -> Option<(NodeId, NodeId)> {
        unsafe { self.vec.get_unchecked(id.to_index()).children }
    }

    /// get next sibling
    #[inline]
    pub fn next_sibling(&self, id: NodeId) -> Option<NodeId> {
        unsafe { self.vec.get_unchecked(id.to_index()).next_sibling }
    }

    /// Sorts the tree depth first
        ...
        K: Ord,
    {
        ...
        self.id_sort_by_key(root_id, &mut f)
    }

    /// Get all child ids in a vec
    #[inline]
    pub fn child_ids(&self, parent: NodeId) -> Vec<NodeId> {
        self.get_unchecked(parent)
            .children()
            .map(|c| c.id())
            .collect()
    }

    /// Sorts the tree depth first (based on id)
    fn id_sort_by_key<K, F>(&mut self, id: NodeId, f: &mut F)
    where
        F: FnMut(&T) -> K,
        K: Ord,
    {
        let mut children = self.child_ids(id);
        children.iter().for_each(|id| self.id_sort_by_key(*id, f));
        if children.len() <= 1 {
            return;
        }
        children.sort_by_key(|node_id| f(&self.vec[node_id.to_index()].value));

        self.vec[id.to_index()].children = Some((children[0], children[children.len() - 1]));

        let mut prev: Option<NodeId> = None;
        for id in children {
            if let Some(prev) = prev {
                self.vec[prev.to_index()].next_sibling = Some(id);
            }
            let node = &mut self.vec[id.to_index()];
    ...
    /// Visits each node, depth first, the result of the func
    /// is accumulated
    pub fn depth_first_fold<Fnode, Acc>(&mut self, mut f: Fnode) -> Acc
    where
        Acc: Default + std::ops::AddAssign<Acc>,
        Fnode: FnMut(&mut T, &mut Acc),
    {
        let root_id = self.root().id();
        self.id_depth_first_fold(root_id, &mut f)
    }

    fn id_depth_first_fold<Fnode, Acc>(&mut self, id: NodeId, f: &mut Fnode) -> Acc
    where
        Acc: Default + std::ops::AddAssign<Acc>,
        Fnode: FnMut(&mut T, &mut Acc),
    {
        let mut child_acc = Acc::default();
        if let Some((mut current, last)) = self.vec[id.to_index()].children {
            loop {
                let acc = self.id_depth_first_fold(current, f);
                child_acc += acc;
                if current == last {
                    break;
                }
                current = self.vec[current.to_index()].next_sibling.unwrap();
            }
        }
        f(self.val_mut(id), &mut child_acc);
        child_acc
    }

    /// go through the node ids in depth-first order
    pub fn depth_first<F>(&mut self, mut f: F)
    where
        F: FnMut(NodeMut<T>),
    {
        let root_id = self.root().id();
        self.id_depth_first(root_id, &mut f)
    }

    /// go through the node ids in depth-first order
    fn id_depth_first<F>(&mut self, id: NodeId, f: &mut F)
    where
        F: FnMut(NodeMut<T>),
    {
        if let Some((mut current, last)) = self.vec[id.to_index()].children {
            loop {
                self.id_depth_first(current, f);
                if current == last {
                    break;
                }
                current = self.vec[current.to_index()].next_sibling.unwrap();
...
impl<'a, T: 'a> NodeRef<'a, T> {
    ...
    pub fn first_child(&self) -> Option<Self> {
        self.node
            .children
            .map(|(id, _)| self.tree.get_unchecked(id))
    }
    ...
    pub fn last_child(&self) -> Option<Self> {
        self.node
            .children
            .map(|(_, id)| self.tree.get_unchecked(id))
    }
...
impl<'a, T: 'a> NodeMut<'a, T> {
    ...
    pub fn first_child(&mut self) -> Option<NodeMut<'_, T>> {
        ...
        ids.map(move |(id, _)| self.tree.get_unchecked_mut(id))
    }

    /// Returns the last child of this node.
    pub fn last_child(&mut self) -> Option<NodeMut<'_, T>> {
        let ids = self.node().children;
        ids.map(move |(_, id)| self.tree.get_unchecked_mut(id))
    }

    /// Returns true if this node has siblings.
    pub fn has_siblings(&self) -> bool {
        self.tree.get_unchecked(self.id).has_siblings()
    }

    /// Returns true if this node has children.
    pub fn has_children(&self) -> bool {
        self.tree.get_unchecked(self.id).has_children()
    }
...
impl<'a, T: 'a> From<NodeMut<'a, T>> for NodeRef<'a, T> {
    fn from(node: NodeMut<'a, T>) -> Self {
        node.tree.get_unchecked(node.id)
    }
}
